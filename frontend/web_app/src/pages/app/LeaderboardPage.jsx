...
export function LeaderboardPage() {
  ...
  const { data, loading } = useAsyncValue(() => mockContentRepository.getLeaderboard(locale), [locale]);
      ...
      <div className="grid lg:grid-cols-[0.9fr,1.1fr]">
        <WorkspaceSection className="border-b border-sand-200 lg:border-b-0 lg:border-r lg:border-sand-200">
          <div className="space-y-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-brand-100 text-brand-700">
              <CrownSimple size={24} weight="duotone" />
            </div>
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-brand-700">
              {locale === "vi" ? "Xếp hạng" : "Leaderboard"}
            </p>
            <h2 className="text-3xl font-semibold tracking-[-0.05em] text-ink-950">{data.heading}</h2>
        ...
        <div className="divide-y divide-sand-200">
          <WorkspaceSection className="border-b border-sand-200">
            <h3 className="text-2xl font-semibold tracking-[-0.04em] text-ink-950">
              {locale === "vi" ? "Nhóm người học nổi bật" : "Standout learners"}
            </h3>
          </WorkspaceSection>

          {data.rows.map((row) => (
            <div key={row.id} className="flex items-center justify-between gap-4 px-5 py-5 sm:px-6 lg:px-8">
              <div className="flex items-center gap-4">
                <div className="flex h-11 w-11 items-center justify-center rounded-full bg-brand-50 text-brand-700">
                  {row.rank <= 3 ? <Trophy size={20} weight="duotone" /> : <Medal size={20} weight="duotone" />}
                </div>
                <div>
              ...
              <div className="text-right">
                <p className="font-mono text-lg font-semibold text-ink-950">{row.xp} XP</p>
