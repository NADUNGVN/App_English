...
export function LearningPage() {
  ...
  const { data, loading } = useAsyncValue(
    () => mockContentRepository.getLearning(locale),
    [locale],
  );
  ...
  const filteredLessons = useMemo(() => {
    ...
    return data.lessons.filter((lesson) => {
      ...
      return `${lesson.title} ${lesson.description} ${lesson.source}`
        .toLowerCase()
        .includes(needle);
    });
  }, [data, deferredSearch]);
      ...
      <WorkspaceSection className="border-b border-sand-200">
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr),320px] xl:items-end">
          <WorkspaceSectionHeader
            ...
            title={
              locale === "vi"
                ? "Chọn một clip ngắn rồi vào việc ngay."
                : "Choose a short clip and get into it quickly."
            }
          ...
          <label className="field-shell xl:pb-1">
            <span className="field-label">
              {locale === "vi" ? "Tìm bài học" : "Find a lesson"}
            </span>
        ...
        {filteredLessons.length === 0 ? (
          ...
          <div className="grid gap-3.5 xl:grid-cols-2">
            {filteredLessons.map((lesson) => (
              <article key={lesson.id} className="surface-panel-soft overflow-hidden p-5">
