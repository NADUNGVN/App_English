...
export function StatisticsPage() {
  ...
  const { data, loading } = useAsyncValue(() => mockContentRepository.getStatistics(locale), [locale]);
      ...
      <section className="grid border-b border-sand-200 sm:grid-cols-2 xl:grid-cols-4 xl:divide-x xl:divide-sand-200">
        {data.summary.map((item, index) => (
          ...
          >
            ...
            <p className="mt-4 font-mono text-3xl font-semibold text-ink-950">
              {item.value}
              <span className="ml-2 font-sans text-base font-medium text-slate-400">{item.suffix}</span>
            </p>
          </div>
        ))}
      </section>
          ...
          <div className="mt-6 grid gap-4">
            {data.bars.map((bar) => (
              <div key={bar.day} className="grid grid-cols-[56px,1fr,48px] items-center gap-3">
                ...
                <span className="text-right font-mono text-sm font-semibold text-ink-950">{bar.minutes}</span>
              </div>
            ))}
          </div>
        </WorkspaceSection>
            ...
            {data.modules.map((module) => (
              <div key={module.label} className="space-y-2">
                <div className="flex items-center justify-between gap-4">
                  <span className="text-sm font-medium text-ink-950">{module.label}</span>
                  <span className="font-mono text-sm font-semibold text-slate-500">{module.value}%</span>
                </div>
