...
import { marketingNavItems } from "../data/marketingContent";
    ...
    <>
      {headerNavItems.map((item) => {
        ...
        return (
          <Link
            key={item.to}
            className={`inline-flex h-12 items-center rounded-full px-5 text-[1.03rem] font-semibold tracking-[-0.02em] transition duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] ${
              isActive
              ...
              {visibleNavItems.map((item) => (
                <Link
                  ...
                  className={`block transition hover:text-ink-950 ${
                    isActivePath(pathname, item.to) ? "text-ink-950" : ""
                  }`}
                  href={item.to}
                >
                  {item.label[locale]}
