...
export function LoginPage() {
  ...
  const copy = {
    ...
    en: {
      ...
      submit: "Log in",
      supabaseError:
        "The Supabase auth project is not active. Restore the Supabase project or update SUPABASE_URL.",
  ...
  const oauthError = useMemo(() => {
    ...
    if (code === "google") {
      return copy.oauthError;
    }

    if (code === "supabase") {
      return (
        copy.supabaseError ??
        "Dich vu dang nhap Supabase dang chua hoat dong. Hay restore project Supabase hoac cap nhat SUPABASE_URL."
      );
    }

    return "";
  }, [copy.oauthError, copy.supabaseError, searchParams]);
    ...
    <AuthSplitShell formTitle={copy.title}>
      <div className="space-y-6">
        <p className="text-[0.95rem] leading-relaxed text-slate-500">
          {copy.switchLead}{" "}
          <Link className="font-semibold text-brand-700 transition hover:text-brand-600" href="/register">
            {copy.switchCta}
          </Link>
        </p>
