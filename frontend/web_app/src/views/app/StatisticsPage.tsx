    ...
    <WorkspaceCanvas>
      <section className="grid border-b border-sand-200 sm:grid-cols-2 xl:grid-cols-4 xl:divide-x xl:divide-sand-200">
        {data.summary.map((item, index) => (
          <div
            key={item.label}
            className={`px-3.5 py-3.5 sm:px-4 lg:px-5 ${
          ...
          >
            ...
            <p className="mt-2.5 font-mono text-[1.6rem] font-semibold text-ink-950">
              {item.value}
              <span className="ml-1.5 font-sans text-[0.875rem] font-medium text-slate-400">
                {item.suffix}
              </span>
            </p>
          </div>
      ...
      <div className="grid lg:grid-cols-[1.12fr,0.88fr]">
        <WorkspaceSection className="border-b border-sand-200 lg:border-b-0 lg:border-r lg:border-sand-200">
          ...
          <div className="mt-4 grid gap-3">
            {data.bars.map((bar) => (
              <div key={bar.day} className="grid grid-cols-[48px,1fr,42px] items-center gap-3">
                <span className="text-[0.8125rem] text-slate-400">{bar.day}</span>
                ...
                <span className="text-right font-mono text-[0.8125rem] font-semibold text-ink-950">
                  {bar.minutes}
                </span>
              </div>
            ))}
            ...
            {data.modules.map((module) => (
              <div key={module.label} className="space-y-2">
                <div className="flex items-center justify-between gap-4">
                  <span className="text-[0.8125rem] font-medium text-ink-950">
                    {module.label}
                  </span>
                  <span className="font-mono text-[0.8125rem] font-semibold text-slate-500">
                    {module.value}%
                  </span>
                </div>
